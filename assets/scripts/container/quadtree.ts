...
export class Quadtree<T> {
    ...
    /** 当前节点对象列表。 */
    private _objects: Array<{ item: T, bounds: Rect }> = [];
    /** 子节点。 */
     ...
     */
    constructor(bounds: Rect, maxObjects: number = 40, maxLevels: number = 3, level: number = 0) {
        this._bounds = bounds;
     ...
     * @param bounds 对象边界矩形
     */
    public insert(item: T, bounds: Rect): void {
        if (this._isSplit) {
     ...
     * @returns 检索结果
     */
    public retrieve(bounds: Rect, result: T[] = []): T[] {
     ...
     */
    private _getIndex(rect: Rect): number {
        let index = -1;
